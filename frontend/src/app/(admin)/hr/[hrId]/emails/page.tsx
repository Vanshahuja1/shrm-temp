...
export default function EmailsPage() {
  ...
  const filteredEmails = emails
    .filter(email => {
      ...
      if (filters.category !== 'all') {
        if (filters.category === 'sent' && email.senderId !== hrId) return false
        
        if (filters.category === 'received') {
          // For received filter, ensure the user is a direct recipient (not CC) and not the sender
          if (email.senderId === hrId) return false
          if (email.cc?.includes(String(hrId)) || email.ccIds?.includes(String(hrId))) return false
          if (!email.recipientIds?.includes(String(hrId))) return false
        }

        if (filters.category === 'cc') {
          // Check both cc and ccIds arrays
          const isInCC = email.cc?.includes(String(hrId)) || email.ccIds?.includes(String(hrId))
  ...
  const fetchEmails = async () => {
    try {
      ...
      setEmails(Array.from(emailMap.values()))
    } catch (err) {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {emails.filter(e => e.senderId === hrId).length} emails
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {emails.filter(e => e.cc?.includes(String(hrId)) || e.ccIds?.includes(String(hrId))).length} emails
                    </p>
                  </div>
                </div>
            ...
            {filteredEmails.length === 0 ? (
              ...
              filteredEmails.map((email) => (
                <div
                  ...
                  className={`p-4 rounded-lg border transition hover:shadow-sm cursor-pointer ${email.isRead ? "bg-gray-50 border-gray-200" : "bg-white border-red-200"
                    }`}
                >
